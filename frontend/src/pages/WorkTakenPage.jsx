...
const WorkTakenPage = () => {
    ...
    // Update pending and balance whenever dependencies change
    useEffect(() => {
        setFormData(prev => ({...prev, amount_pending: balance, balance: balance }));
    }, [balance]);
    ...
    const fetchWorks = async () => {
        ...
        try {
            ...
            setWorks(data);
        } catch (error) { setMessage('Failed to fetch work records.'); }
    ...
    const handleCitizenSelect = async (selectedOption) => {
        ...
        try {
            ...
            setFormData(prev => ({ ...initialFormState, citizen_id: citizen.id, citizen_name: citizen.name, mobile_no: citizen.mobile_no }));
            setCitizenVehicles(citizen.vehicles || []);
            setSelectedVehicle(null);
        } catch (error) { setMessage('Error fetching citizen details.'); }
    };
    ...
    const handleVehicleSelect = async (selectedOption) => {
        ...
        try {
            ...
            setFormData(prev => ({ ...prev, citizen_id: vehicle.citizen.id, citizen_name: vehicle.citizen.name, mobile_no: vehicle.citizen.mobile_no, vehicle_id: vehicle.id, vehicle_no: vehicle.registration_no, }));
            setCitizenVehicles(vehicle.citizen.vehicles || []);
            setSelectedCitizen({ value: vehicle.citizen.id, label: `${vehicle.citizen.name} (${vehicle.citizen.mobile_no})` });
        } catch (error) { setMessage('Error fetching vehicle details.'); }
    };

    const handleManualVehicleSelect = (e) => {
        const vehicleId = e.target.value;
        const selectedVehicle = citizenVehicles.find(v => v.id == vehicleId);
        setFormData(prev => ({ ...prev, vehicle_id: selectedVehicle ? selectedVehicle.id : '', vehicle_no: selectedVehicle ? selectedVehicle.registration_no : '' }));
    };
    ...
    const startEdit = (work) => {
        setEditId(work.id);
        if (work.citizen?.id) {
             apiClient.get(`/citizens/${work.citizen.id}`).then(res => { setCitizenVehicles(res.data.vehicles || []); });
        }
        setFormData({ ...initialFormState, ...work, services_list: work.services_list ? work.services_list.map(val => ({ value: val, label: val })) : [], citizen_name: work.citizen?.name || '', vehicle_no: work.vehicle?.registration_no || '', mobile_no: work.citizen?.mobile_no || '' });
        setSelectedCitizen(work.citizen ? { value: work.citizen.id, label: `${work.citizen.name} (${work.citizen.mobile_no})` } : null);
            ...
            <div className="card p-3 mb-4">
                ...
                <form onSubmit={handleSubmit}>
                     ...
                     <div className="row g-3">
                        ...
                        <div className="col-md-3"><label className="form-label">Amount Pending</label><input type="number" step="0.01" name="amount_pending" value={formData.amount_pending || ''} className="form-control" readOnly/></div>
