...
const DocumentSection = ({ vehicleId, docType, title, fields, fetchCount }) => {
  ...
  const startEdit = (record) => {
      ...
      // Format dates correctly for the form input fields
      const formattedRecord = { ...record };
      fields.forEach(field => {
          if (field.type === 'date' && formattedRecord[field.name]) {
              formattedRecord[field.name] = formattedRecord[field.name].split('T')[0];
          }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if(editRecordId) { data.append('_method', 'PUT'); }

    // For updates, the endpoint is different because of how FormData works with PUT in some frameworks
    const url = editRecordId ? `/vehicles/${vehicleId}/${docType}/${editRecordId}` : endpoint;
  ...
  return (
    <div>
      ...
      {/* ✅ RESTORED: Full JSX for the form */}
      {showForm && (
      ...
      {/* ✅ RESTORED: Full JSX for the table */}
      <div className="table-responsive">
        <table className="table table-sm table-bordered table-striped">
            ...
            {records.length > 0 ? records.map(rec => (
                <tr key={rec.id}>
                {fields.map(f => <td key={f.name}>{rec[f.name] || '-'}</td>)}
                <td>
                    {rec.file_path ? (
